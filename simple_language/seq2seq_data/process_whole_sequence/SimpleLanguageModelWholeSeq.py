...
class SimpleLanguageModelWholeSeq(Model):
    def __init__(self, d_model, vocab_size, tokenizer, path_to_model=None, bert_trainable=True):
        super(SimpleLanguageModelWholeSeq, self).__init__()
    ...
    def train_step(self, inputs):
        ...
        with tf.GradientTape() as tape:
            ...
            new_tokens = tokens['input_ids']
            # max_length = tf.shape(tokens)[1]
            # for t in tf.range(max_length-1):
            # new_tokens = tokens
            loss, _ = self._loop_step(new_tokens, inp['attention_mask'],
    ...
    def _loop_step(self, new_tokens, input_mask, enc_output, dec_state):
        new_input, target = new_tokens[:, 0:1], new_tokens[:, 1:]
        # print(new_input)
        # print(target)

        # print(text[:1])
        input_token = tf.concat([new_input, tf.zeros(shape=(tf.shape(new_input)[0], 127), dtype=tf.int32)], -1)
        # print(input_token.shape)

        target_token = tf.concat([target, tf.zeros(shape=(tf.shape(target)[0], 1), dtype=tf.int32)], -1)
